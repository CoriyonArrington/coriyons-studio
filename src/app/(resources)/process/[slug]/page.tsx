...
import { 
    Box, VStack, Image, UnorderedList, ListItem, SimpleGrid, 
    Divider, HStack, Heading, Text, Icon
...
// A simple, local function to render an icon.
...
export default async function ProcessStepDetailPage({ params }: ProcessStepDetailPageProps) {
  const { slug } = params;
  ...
  let previousStepLink: NavLinkInfo | undefined, nextStepLink: NavLinkInfo | undefined;
        ...
        <VStack spacing={8} alignItems="stretch" maxW="container.lg" mx="auto">
          <VStack spacing={3} alignItems="center" textAlign="center" mb={8}>
            <HStack spacing={4} alignItems="center">
              {renderIcon(icon?.name, { size: 40, color: "var(--chakra-colors-primary-500)", strokeWidth: 2.5 })}
                    ...
                    <Box key={index} mb={5} p={4} borderWidth="1px" borderRadius="md" borderColor="border">
                      ...
                      {subStep.items && subStep.items.length > 0 && (
                        <UnorderedList spacing={1} pl={5} mt={2}>
                          {subStep.items.map((item, itemIndex) => <ListItem key={itemIndex}>{item}</ListItem>)}
                        </UnorderedList>
                      )}
                    </Box>
                  ))}
              ...
              {detailContent.insights?.items && detailContent.insights.items.length > 0 && (
                <Box mt={4}>
                  ...
                  <UnorderedList spacing={1} pl={5}>
                    {detailContent.insights.items.map((item, index) => <ListItem key={index}>{item}</ListItem>)}
                  </UnorderedList>
                </Box>
              )}
              {detailContent.visuals && detailContent.visuals.length > 0 && (
                <Box mt={6}>
                  ...
                  <SimpleGrid columns={{ base: 1, md: detailContent.visuals.length > 1 ? 2 : 1 }} spacing={6}>
                    {detailContent.visuals.map((visual: ContentVisual, index: number) => visual.url ? <VStack key={index} spacing={2} alignItems="center"><Image src={visual.url} alt={visual.alt || `Process visual ${String(index + 1)}`} borderRadius="md" boxShadow="sm" maxH="400px" objectFit="contain"/>{visual.caption && <Text fontSize="sm" color="muted.foreground" textAlign="center">{visual.caption}</Text>}</VStack> : null)}
                  </SimpleGrid>
                </Box>
              )}
...
export async function generateMetadata({ params }: ProcessStepDetailPageProps, _parent: ResolvingMetadata): Promise<Metadata> {
  const { slug } = params;
  const step = await getProcessStepBySlug(slug); 
  if (!step) return { title: 'Process Step Not Found', description: 'The requested process step could not be found.' };
  
  const description = step.description || `Learn about the ${step.title} phase of our design process at Coriyon's Studio.`;
  const openGraphImages: { url: string | URL; alt?: string }[] = []; 
  
  if (step.content?.visuals?.[0]?.url) {
  ...
  return {
    ...
    description,
    openGraph: { title: `${step.title} | Our Process`, description, type: 'article', url: `/process/${slug}`, images: openGraphImages.length > 0 ? openGraphImages : undefined }, 
  };
}